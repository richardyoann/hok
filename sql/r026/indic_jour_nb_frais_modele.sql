...
join use_case_specif.vue_dfa_imi_postes p on
	win.alm_asset_asset_tag_display_value = p.alm_asset_asset_tag_display_value
...
select
	...
	code_moa,
	case
		when mecm."Description_equipement_informatique" is null
			or cmdb_model_model_number_display_value is null
			or trim(mecm."Description_equipement_informatique" ) = ''
				or trim(cmdb_model_model_number_display_value) = '' 
then 3
...
select
	'Nb postes modele frais' as indicateur,
	...
	sum(case when j_modele = 1 and vu = 'vu' then 1 else 0 end ) as valeur
from
	justesse f
group by
