...
select
	...
	uti.code_moa,
	case
		when win.nni like '%-OP%'
			or win.nni like '%AUTRE%'
			or win.nni like '%-G%'
			or win.nni like '%-PR%'
			or win.nni like '%CSP%'
	then 'Exclu'
...
select
	...
	code_moa,
	case
		when mecm.user_top_nni_exclu = 'Exclu'
			or i.nni_exclu = 'Exclu' then 4
			when mecm.user_top_nni_exclu is null
			or nni_exclu is null
			or trim(mecm.user_top_nni_exclu) = ''
				or trim(nni_exclu) = ''
then 3
				when mecm.user_top_nni_exclu = nni_exclu then 1
				else 2
			end as j_utilisateur ,
			vu
		from
			itam i
		left join mecm on
...
select
	'Nb postes utilisateur frais' as indicateur,
	...
	sum(case when j_utilisateur = 1 and vu = 'vu' then 1 else 0 end ) as valeur
from
	justesse f
group by
