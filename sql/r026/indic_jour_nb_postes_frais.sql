...
-- CTE pour lister les postes de MECM et les données essentiels au traitement
mecm AS (
	SELECT
		DISTINCT ON
...
-- CTE pour lister les postes se trouvant dans ITAM
itam AS (
	SELECT
...
-- CTE permettant de calculer la fraicheur
fraicheur AS (
	SELECT
		...
		JOIN use_case_specif.vue_lastscan s 
			ON 	s.hostname = i.alm_asset_asset_tag_display_value
		WHERE  (current_date ::date - s.lastscan ::date ) ::integer >= 61	
) 
-- Requête principale pour obtenir les statistiques de la fraicheur des postes
SELECT
	'Nb postes frais' AS indicateur,
	...
	-- fraicheur : poste ayant modele, entite_utilisateur, fabriquant et virtualisation à 1 
