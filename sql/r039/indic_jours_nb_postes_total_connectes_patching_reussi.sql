...
-- Requête principale pour obtenir les statistiques de déploiement des postes connectés
SELECT
	pc.campagne || ' - ' || pc.historisation  ||' -Total connectés' AS indicateur,
	pc.campagne || ' - ' || pc.historisation  ||' -Total connectés' as indicateur_parent,
	concat(u.direction, '/' , u.division , '/' , u.regroupement_unite , '/' , u.um , '/' , u.dum , '/' , u.code_moa) maille,
	'groupes mailles direction/division/regroupement_unite/um/dum/code rc' as maille_parent,
    -- Count le nombre total pc en excluant les pc non ciblé
	COUNT(pc.computer) FILTER (WHERE pc.summarizationstate != 2) as valeur
FROM utilisateurs u
INNER JOIN postes_campagnes pc ON u."Matricule__NNI_" = pc.utilisateur 
WHERE pc.historisation NOT IN ('G3','G2')
--Filtre permettant de prendre que les postes connectés
AND pc.cmdb_ci_actif_u_network_access_display_value <> 'Hors RLE'::text 
GROUP BY
	concat(u.direction, '/' , u.division , '/' , u.regroupement_unite , '/' , u.um , '/' , u.dum , '/' , u.code_moa),pc.campagne,pc.historisation;